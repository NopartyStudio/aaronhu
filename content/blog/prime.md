...
这里还有一个比较直观的证明：
> 当 `i % prime[j] == 0` 时，说明 `prime[j]` 是 `i` 的一个质因子。由于内层循环按从小到大遍历质数，所以 `prime[j]` 就是 `i` 的**最小质因子**。

> 此时如果继续往下走，用 `prime[j+1]` 去筛 `prime[j+1] * i`，会出问题——因为 `prime[j+1] > prime[j]`，而 `prime[j]` 整除 `i`，所以 `prime[j]` 也整除 `prime[j+1] * i`。这意味着 `prime[j+1] * i` 的最小质因子其实是 `prime[j]`，而不是 `prime[j+1]`。

> 如果现在用 `prime[j+1]` 筛掉了它，那它就不是被自己的最小质因子筛掉的，后面还会被重复筛一次，线性复杂度就保不住了。

> **所以 break 的本质是：保证每个合数只被它的最小质因子筛掉恰好一次。**
